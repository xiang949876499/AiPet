...
class AdvisorAgent(BaseAgent):
    ...
    def execute(self, context: dict) -> dict:
        ...
        category = str(context.get("category") or "通用经营问题").strip() or "通用经营问题"
        ...
        fallback = (
            f"问题判断：这是一个{category}问题，先把它拆成客户真实需求、门店经营目标和今天能推进的动作。\n\n"
            "建议动作：先选一个最小动作执行，例如筛选 20 位重点客户、准备 1 条朋友圈内容，"
            "或设计 1 个低门槛到店理由；当天记录触达数、回复数、到店数，复盘哪句话有效；"
            "未回复客户隔 2-3 天轻提醒一次，避免连续打扰。\n\n"
            "可复制话术：您好，想和您确认一下毛孩子最近的护理安排。我们这边可以先帮您做基础评估，"
            "再根据实际情况推荐合适方案，不会让您多花没必要的钱。\n\n"
            "注意事项：需要人工判断后再执行，不自动群发，不承诺效果，不涉及医疗诊断或用药建议。"
        )
        prompt = (
            "你是宠物门店运营专家助手。你的任务是根据店主的提问，\n"
            "按指定场景给出结构化的、可立即执行的建议，避免答非所问。\n\n"
            f"场景分类：{category}\n"
            f"店主问题：{question}\n\n"
            "要求：\n"
            "1. 先判断问题属于该场景下的哪类经营问题，不要泛泛回答。\n"
            "2. 建议要具体、可执行，优先给今天可以做的动作。\n"
            "3. 涉及客户沟通、内容营销或活动推广时，提供可直接复制使用的话术。\n"
            "4. 不做医疗诊断，不承诺治疗效果，不建议用药。\n"
            "5. 语气亲切专业，像资深宠物店长在分享经验。\n"
            "6. 回答控制在 200-400 字，严格使用以下四段标题：\n"
            "问题判断：\n"
            "建议动作：\n"
            "可复制话术：\n"
            "注意事项："
